...
from examples.postdoc.risk_aware_rl.api.utils.dataset_types import Track, MotionState
...
def update_objects_plot(timestamp,
                        track_dict,
                        patches_dict,
                        text_dict,
                        ego_rect=None,
    ...
    if ego_rect is None:
        if axes is not None:
            ego_rect = matplotlib.patches.Polygon(polygon_xy_from_motionstate(ego_track.motion_state, ego_track.width, ego_track.length), closed=True, zorder=20, color='red')
            axes.add_patch(ego_rect)
            ego_text=axes.text(ego_track.motion_state.x, ego_track.motion_state.y+2, 'ego', horizontalalignment='center', zorder=30)
    else:
        ego_rect.set_xy(polygon_xy_from_motionstate(ego_track.motion_state, ego_track.width, ego_track.length))
        ego_text.set_position((ego_track.motion_state.x, ego_track.motion_state.y + 2))
